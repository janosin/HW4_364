...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/janosinHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
def get_or_create_search_term(term):
    ...
    t = SearchTerm.query.filter_by(term=term).first()
    if t:
        return t
...
def get_or_create_collection(db_session, name, current_user, gif_list=[]):
    ...
    coll = db_session.query(PersonalGifCollection).filter_by(name=name, user_id=current_user.id).first()
    if coll:
        return coll
    
    else:
        coll = PersonalGifCollection(name=name, user_id=current_user.id, gifs=[])
        for gif in gif_list:
            coll.gifs.append(gif)
        db_session.add(coll)
        db_session.commit()
        return coll
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        for gif in form.gif_picks.data:
            gif_list.append(get_gif_by_id(gif))
